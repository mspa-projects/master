# -*- coding: utf-8 -*-
'''
This module implements the K-means (random) Model of clustering
...
Output:
    ...
    score: silhouette coefficient for K-means (random) model
...
# This module builds the K-means (random) model, fits to the training data
# and returns the silhouette score
def buildKmeansRandomModel(
		trainingData,
    ...
    # max_iter: is max number of iterations to be performed
    # n_init: number of initializations to perform. the best result is kept.
    clf = KMeans(init='random', n_clusters=clusterComp, max_iter = 100, n_init=10)
    clf.fit(X)
...
def main():
    df_countyCluster, score = buildKmeansRandomModel(
